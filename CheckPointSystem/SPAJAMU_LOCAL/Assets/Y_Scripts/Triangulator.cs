...
public class Triangulator
{
    ...
    public int[] Triangulate ()
	{
		...
		int i1 = indices [1];
		int i2 = indices [2];
		Vector3 pos0 = new Vector3 (_points [i0].x, _points [i0].y, 0f);
		Vector3 pos1 = new Vector3 (_points [i1].x, _points [i1].y, 0f);
		Vector3 pos2 = new Vector3 (_points [i2].x, _points [i2].y, 0f);
		Vector3 v1 = pos1 - pos0;
		Vector3 v2 = pos2 - pos1;
		Vector3 crossVec = Vector3.Cross (v1, v2);
		if (Vector3.Dot (_cameraPosition, crossVec) > 0) {
			reversFlag = false;
		}
		if (reversFlag) {
			indices.Reverse ();
